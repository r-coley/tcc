	...
	while (lexer_peek()->kind != TOK_RBRACE) {
		...
		if (field->is_struct) {
			if (lexer_peek()->kind == TOK_LBRACE) {
				...
				/* Parenthesized compound literals, e.g. (struct S){...}
				 * or (typedef_name){...}, are also single struct values. */
				if (lexer_peek()->kind == TOK_LPAREN) {
					const Token *type_tok = lexer_peek_after_next();
					if (type_tok->kind == TOK_STRUCT ||
					    (type_tok->kind == TOK_IDENT &&
					     find_typedef(type_tok->text) &&
					     find_typedef(type_tok->text)->kind == TY_STRUCT))
						copy_struct_object = 1;
				}

				if (copy_struct_object) {

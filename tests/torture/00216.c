...
void sys_ni(void) { fprintf(stderr,"ni\n"); }
void sys_one(void) { fprintf(stderr,"one\n"); }
void sys_two(void) { fprintf(stderr,"two\n"); }
void sys_three(void) { fprintf(stderr,"three\n"); }
...
test_correct_filling (struct SEA *x)
{
  ...
  if (x->i != 0 || x->j != 5 || x->k != 0 || x->l != 0)
    fprintf(stderr,"sea_fill%d: wrong\n", i);
  else
    fprintf(stderr,"sea_fill%d: okay\n", i);
  i++;
}

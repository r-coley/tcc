...
#define USER_LABEL_MAX 256
typedef struct UserLabel {
	const char *name;
	int id;
} UserLabel;

static UserLabel user_labels[USER_LABEL_MAX];
...
{
	/*
	 * C labels have function scope.  Use backend numeric labels instead of
	 * emitting source names like Llabel, otherwise the same label name in two
	 * different functions collides at assembly time.
	 *
	 * The table is reset for each function, while new_label() remains
	 * translation-unit global, so each source label gets one unique assembler
	 * label and every goto in that function branches to the same label.
	 */
	for (int i = 0; i < user_label_count; i++) {
		if (STRCMP(user_labels[i].name, name) == 0)
			return user_labels[i].id;
	}
	...
	int id = new_label();
	user_labels[user_label_count].name = name;
	user_labels[user_label_count].id = id;
	user_label_count++;
	return id;
}
...
emit_expr(Node *node, Codegen *cg)
{
	...
	case ND_CALL: {
		...
		cg->emit_prepare_call_args(count, fixed_params);
		if (node->left)
			cg->emit_call_saved();
		else
			cg->emit_call(node->name);
		cg->emit_cleanup_call_args(count, fixed_params);
		return;
	}
				...
				if (globals[i].is_struct) {
					...
					if (globals[i].init_count > 0) {
						...
						for (int j = 0; j < total; j++) {
							/* Pointer/function-pointer fields in global struct aggregates are
							 * recorded by the parser in init_syms[] using 8-byte slots.
							 * Emit a relocation instead of eight zero bytes. */

...
static void mips_stack_save_acc(void) {
    printf("    move $v0, $sp\n");
}

static void mips_stack_restore_acc(void) {
    printf("    move $sp, $v0\n");
}

static void mips_stack_alloc_acc(void) {
    printf("    addiu $v0, $v0, 15\n");
    printf("    li $t2, -8\n");
    printf("    and $v0, $v0, $t2\n");
    printf("    subu $sp, $sp, $v0\n");
    printf("    move $v0, $sp\n");
}

static void mips_load_imm(long value) {
    printf("    li $v0, %ld\n", value);
}
...
static void mips_udiv_op(void);
static void mips_umod_op(void);
...
static void mips_i64_helper_call(const char *name) {
    printf("    # i64 helper-call lowering scaffold\n");
    printf("    jal %s\n", name);
    printf("    nop\n");
}

static void mips_binop_sized(const char *op, int size) {
    if (size == 8) {
        if (STRCMP(op, "mul") == 0) {
            mips_i64_helper_call("__tcc_mips_i64_mul");
            return;
        }
        if (STRCMP(op, "div") == 0) {
            mips_i64_helper_call("__tcc_mips_i64_div");
            return;
        }
        if (STRCMP(op, "mod") == 0) {
            mips_i64_helper_call("__tcc_mips_i64_mod");
            return;
        }
        if (STRCMP(op, "udiv") == 0) {
            mips_i64_helper_call("__tcc_mips_u64_div");
            return;
        }
        if (STRCMP(op, "umod") == 0) {
            mips_i64_helper_call("__tcc_mips_u64_mod");
            return;
        }
    }
...
static void mips_ushr_op(void) { printf("    srlv $v0, $t0, $v0\n"); }
static void mips_shl_imm_op(int imm) { printf("    sll $v0, $v0, %d\n", imm); }
static void mips_shr_imm_op(int imm) { printf("    sra $v0, $v0, %d\n", imm); }
static void mips_ushr_imm_op(int imm) { printf("    srl $v0, $v0, %d\n", imm); }

static void mips_cast_op(int size, int is_unsigned) {
    if (size == 1) {
        if (is_unsigned)
...
static void mips_load_global(const char *name, int size) {
    ...
    case 8:
        /*
         * MIPS is currently a 32-bit backend. Some front-end pointer
         * expressions can still arrive here with size 8, but pointer
         * values are represented as one MIPS32 word in this backend.
         */
        printf("    lw $v0, 0($t0)\n");
        break;
    default:
...
static void mips_load_global_member(const char *name, int offset, int size, int is_extern) {
    (void)is_extern;
    printf("    la $t0, %s\n", name);
    if (offset)
    ...
    case 8:
        ...
        break;
    default:
        ICE("unsupported global member load size for MIPS");
        break;
    }
}
...
static void mips_ptr_copy(int size) {
    int off = 0;
...
static void mips_store_local_ptr_member(int local_offset, int member_offset, int size) {
    printf("    lw $t0, %d($fp)\n", local_offset);
    switch (size) {
    case 1:
        printf("    sb $v0, %d($t0)\n", member_offset);
    ...
    case 8:
        ...
        break;
    default:
        ICE("unsupported local ptr member store size for MIPS");
        break;
    }
}

static void mips_string_literal(int label, const char *value, size_t len, int width) {
    if (width > 1)
...
static void mips_load_global_extern(const char *name) {
    mips_load_global(name, 4);  /* MIPS32 extern refs are pointer-sized words */
}
static void mips_store_global_extern(const char *name) {
    mips_store_global(name, 4);  /* MIPS32 extern refs are pointer-sized words */
}
...
Codegen mips_codegen = {
    ...
    mips_stack_alloc,
    mips_stack_save_acc,
    mips_stack_restore_acc,
    mips_stack_alloc_acc,
    mips_load_imm,
    mips_push_acc,
    mips_pop_to_tmp,
    mips_pop_to_acc,
    ...
    NULL,
    NULL,
    .emit_binop_sized = mips_binop_sized,
    .emit_load_global_member = mips_load_global_member,
    .emit_store_local_ptr_member = mips_store_local_ptr_member
};

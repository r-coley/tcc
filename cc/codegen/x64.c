...
static void x64_load_imm(int value) {
    printf("    mov rax, %d\n", value);
}
...
static void x64_cast_op(int size) {
    if (size == 1)
        printf("    and rax, 255\n");
}
...
static void x64_store_param(int index, int offset) {
    static const char *regs[] = {"rdi", "rsi", "rdx", "rcx", "r8", "r9"};
    if (index < 0 || index >= 6) {
        ICE("x64 backend supports only 6 integer/pointer parameters");
        return;
    }

    printf("    mov QWORD PTR %s, %s\n", x64_slot(offset), regs[index]);
}
...
static void x64_ptr_copy(int size) { (void)size; }
...
static void x64_string_literal(int label, const char *value) {
...
Codegen x64_codegen = {
    ...
    NULL,               /* emit_source_loc: not implemented for x64 */
    x64_cmp_lt_u,
    x64_cmp_le_u,
    x64_cmp_gt_u,
    x64_cmp_ge_u

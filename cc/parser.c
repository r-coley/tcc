...
static Node *append_struct_copy_from_ptr_fields_at(Node *head, int dst_base, Node *src_base, const char *struct_name, int src_base_offset);
...
static int is_global_struct(const char *name);
...
static Node *clone_node_tree(Node *node);
...
static Node *append_local_zero_fill(Node *head, const char *name, int offset, int bytes);
...
parse_local_scalar_initializer_expr(int target_size)
{
	...
	if (target_size == 8) {
		if (lexer_peek()->kind == TOK_AMP &&
		    ...
		    find_func(lexer_peek_next()->text)) {
			lexer_next(); /* & */
			const Token *fn = lexer_peek();
			lexer_next();
			return new_func_addr(fn->text);
		}
...
parse_struct_initializer_values(StructDef *def, const char *struct_name, int base_offset, Node *head)
{
	...
	Debug(1, "STRUCT_INIT_ENTER struct=%s base=%d tok=%s text=%s next=%s next_text=%s\n",
	      ...
	      token_debug_name(lexer_peek_next()->kind),
	      lexer_peek_next()->text ? lexer_peek_next()->text : "");
	...
	while (lexer_peek()->kind != TOK_RBRACE) {
		...
		if (lexer_peek()->kind == TOK_DOT) {
			...
			if (nested_designator) {
				...
				Node *member = new_member(field->name, designator_base + field->offset);
				member->elem_size = field->size;
				member->type = type_for_size(field->size);
				STRNCPY(member->struct_name, designator_struct_name, sizeof(member->struct_name) - 1);
				head = append_node(head, new_assign(member, parse_local_scalar_initializer_expr(member->elem_size)));
		...
		if (field->is_struct) {
			if (lexer_peek()->kind == TOK_LBRACE) {
				...
				Debug(1, "STRUCT_INIT_BRACED_STRUCT_BEFORE_RBRACE parent=%s field=%s nested=%s tok=%s text=%s next=%s next_text=%s\n",
				      ...
				      lexer_peek_next()->text ? lexer_peek_next()->text : "");
				expect(TOK_RBRACE);
				Debug(1, "STRUCT_INIT_BRACED_STRUCT_AFTER_RBRACE parent=%s field=%s nested=%s tok=%s text=%s next=%s next_text=%s field_index=%d/%d\n",
				      struct_name ? struct_name : "<anon>",
				      field->name[0] ? field->name : "<anon>",
				      field->struct_name[0] ? field->struct_name : "<anon>",
				      token_debug_name(lexer_peek()->kind),
				...
				if (lexer_peek()->kind == TOK_COMMA &&
				    ...
				    lexer_peek_next()->kind != TOK_RBRACE) {
				 ...
				 */
				if (lexer_peek()->kind == TOK_IDENT &&
				    lexer_peek_next()->kind == TOK_ARROW &&
				    lexer_peek_after_next()->kind == TOK_IDENT) {
					/*
					...
					const Token *src_field_tok = lexer_peek_after_next();
					...
					if (src_base->type && src_base->type->kind == TY_PTR &&
						...
						src_struct_name = src_base->type->base->struct_name;
					...
					lexer_next(); /* struct tag */
					expect(TOK_RPAREN);

					if (STRCMP(compound_struct_name->text, field->struct_name) != 0)
						fatal_cur("Struct compound literal initializer type mismatch\n");
					 ...
					 */
					if (lexer_peek()->kind == TOK_RBRACE &&
					    lexer_peek_next()->kind == TOK_COMMA &&
					    field_index < def->field_count) {
			...
			if (lexer_peek()->kind == TOK_LBRACKET) {
				while (lexer_peek()->kind == TOK_LBRACKET) {
					...
					const Token *lo_tok = lexer_peek();
					if (lo_tok->kind != TOK_NUM)
						fatal_cur("Expected constant array designator index\n");
					int lo = (int)lo_tok->value;
					int hi = lo;
					lexer_next();
					...
					if (lexer_peek()->kind == TOK_DOT &&
						...
						const Token *hi_tok = lexer_peek();
						if (hi_tok->kind != TOK_NUM)
							fatal_cur("Expected constant array designator range end\n");
						hi = (int)hi_tok->value;
						lexer_next();
					}
					...
					if (lo < 0 || hi < lo || hi >= arr_max)
						fatal_cur("Array designator index out of range\n");

					Node *rhs = parse_expr();
					for (int di = lo; di <= hi; di++) {
						Node *elem = new_member(field->name, arr_offset + di * field->elem_size);
						elem->elem_size = field->elem_size;
						elem->type = type_for_size(field->elem_size);
						/*
						 * A range designator initializes multiple elements from the
						 * same expression, but each assignment node must own a distinct
						 * RHS tree.  Reusing rhs for every element makes free_ast() walk
						 * and free the same AST more than once.
						 */
						Node *elem_rhs = (di == lo) ? rhs : clone_node_tree(rhs);
						head = append_node(head, new_assign(elem, elem_rhs));
					}
				...
				while (lexer_peek()->kind != TOK_RBRACE && ae < arr_max) {
					...
					if (lexer_peek()->kind == TOK_LBRACKET) {
						...
						for (int di = lo; di <= hi; di++) {
							...
							elem->elem_size = field->elem_size;
							elem->type = type_for_size(field->elem_size);
							head = append_node(head, new_assign(elem, rhs));
						}
					...
					elem->elem_size = field->elem_size;
					elem->type = type_for_size(field->elem_size);
					head = append_node(head, new_assign(elem, parse_local_scalar_initializer_expr(elem->elem_size)));
      ...
      lexer_peek_next()->text ? lexer_peek_next()->text : "");
		...
		if (lexer_peek()->kind == TOK_COMMA) {
			...
			if (field_index >= def->field_count &&
			    lexer_peek_next()->kind != TOK_RBRACE &&
			    lexer_peek_next()->kind != TOK_DOT &&
			    lexer_peek_next()->kind != TOK_LBRACKET)
				break;
        ...
        Node *decl_node)
{
	...
	while (lexer_peek()->kind != TOK_RBRACE) {
		...
		if (lexer_peek()->kind == TOK_LBRACE) {
			...
			expect(TOK_RBRACE);
		           ...
		           lexer_peek_third()->kind == TOK_RPAREN &&
		           lexer_peek_fourth()->kind == TOK_LBRACE) {
			/* (struct T){ ... } as a whole array element. */
			lexer_next(); /* ( */
			lexer_next(); /* struct */
			const Token *compound_struct_name = lexer_peek();
			lexer_next(); /* tag */
			...
			if (STRCMP(compound_struct_name->text, struct_name) != 0)
				fatal_cur("Struct array compound literal initializer type mismatch\n");
			expect(TOK_LBRACE);
			...
			Node *lhs = new_var(var_name, elem_offset);
			lhs->type = type_struct(struct_name, def->size);
			lhs->elem_size = def->size;
			STRNCPY(lhs->struct_name, struct_name, sizeof(lhs->struct_name) - 1);
			Node *rhs = parse_expr();
			head = append_node(head, new_struct_assign(lhs, rhs, def->size));
		} else {
	...
	if (infer_len) {
		...
		for (int i = 0; i < local_count; i++) {
			if (STRCMP(locals[i].name, var_name) == 0) {
				locals[i].array_len = max_used;
				if (locals[i].type)
					locals[i].type->array_len = max_used;
				break;
			}
		}
	}
...
parse_struct_initializer_block(const char *var_name, const char *struct_name, int base_offset, Node *decl_node)
{
	...
	head = parse_struct_initializer_values(def, struct_name, base_offset, head);
...
{
	if (offset < 0 || offset + 8 > 1024) {
		fatal_cur("Global symbol initializer too large\n");
		exit(1);
	}
	...
	int slot = offset / 8;
	if (slot >= 0 && slot < 128) {
		STRNCPY(g->init_syms[slot], sym, sizeof(g->init_syms[slot]) - 1);
		g->init_syms[slot][sizeof(g->init_syms[slot]) - 1] = '\0';
		if (g->init_sym_count <= slot)
			g->init_sym_count = slot + 1;
	}

	if (g->init_count < offset + 8)
		g->init_count = offset + 8;
}
...
make_var_node(const char *name)
{
	 ...
	 *     }
	 *
	 * The previous make_scalar_var_node() checked globals before locals, so the
	 * initializer helper built loads from the global _phdr object instead of the
	 * local pointer phdr.  That produced code which loaded the first four bytes
	 * of _phdr (0x03040506) and then dereferenced that value as an address.
	 */
	for (int i = local_count - 1; i >= 0; i--) {
		if (STRCMP(locals[i].name, name) == 0) {
			Node *node = new_var(name, locals[i].offset);
			node->type = locals[i].type ? locals[i].type : type_int();
			node->is_unsigned = node->type && node->type->is_unsigned;

			if (locals[i].is_array) {
				Node *addr = new_addr(node);
				addr->is_pointer = 1;
				addr->elem_size = locals[i].elem_size ? locals[i].elem_size : 4;
				if (node->type && node->type->kind == TY_ARRAY)
					addr->type = type_ptr(node->type->base);
				return addr;
			}

			node->is_pointer = locals[i].is_pointer;
			node->elem_size = locals[i].elem_size ? locals[i].elem_size : 4;
			if (locals[i].struct_name[0])
				STRNCPY(node->struct_name, locals[i].struct_name, sizeof(node->struct_name) - 1);
			return node;
		}
	}
...
make_scalar_var_node(const char *name)
{
	...
	/*
	 * Locals and parameters must shadow globals.  This function is used by
	 * expression parsing and by aggregate-initializer copy helpers, so the
	 * lookup order matters for code like:
	 *
	 *     struct pkthdr phdr;
	 *     void foo(struct pkthdr *phdr_) {
	 ...
	 *     }
	 *
	 * The local pointer phdr must win over the global object phdr.
	 */
	for (int i = local_count - 1; i >= 0; i--) {
		if (STRCMP(locals[i].name, name) == 0) {
			Node *node = new_var(name, locals[i].offset);
...
append_local_zero_fill(Node *head, const char *name, int offset, int bytes)
{
	for (int i = 0; i < bytes; i++)
		head = append_node(head, make_local_array_store(name, offset, 1, i, 0));
	return head;
}
...
parse_function_pointer_declaration_statement(void)
{
	...
	if (ret->kind != TOK_INT && ret->kind != TOK_CHAR && ret->kind != TOK_VOID) {
		return NULL;
	}
	...
	if (lexer_peek()->kind == TOK_LBRACKET) {
		...
		if (lexer_peek()->kind == TOK_ASSIGN) {
			...
			if (lexer_peek()->kind == TOK_LBRACE) {
				...
				while (lexer_peek()->kind != TOK_RBRACE) {
					int first_index = g->init_count;
					int last_index = g->init_count;
					int has_designator = 0;

					/* Generic global array designators, including ranges:
					 *
					 *   const fptr table[3] = {
					 *       [0 ... 2] = &sys_ni,
					 *       [0] = sys_one,
					...
					if (lexer_peek()->kind == TOK_LBRACKET) {
						lexer_next(); /* [ */
						if (lexer_peek()->kind != TOK_NUM)
							fatal_cur("Global array designator index must be constant integer\n");
						first_index = lexer_peek()->value;
						...
						if (lexer_peek()->kind == TOK_DOT &&
							...
							lexer_next();
							if (lexer_peek()->kind != TOK_NUM)
								fatal_cur("Global array designator range end must be constant integer\n");
							last_index = lexer_peek()->value;
							lexer_next();
						}
					...
					if (g->array_len > 0 && last_index >= g->array_len)
						fatal_cur("Global array designator index out of range\n");
					if (last_index >= 1024 || last_index >= 128)
						fatal_cur("Too many global array initializers\n");
					...
					if (try_parse_null_pointer_constant()) {
						...
						lexer_next();
					} else if (lexer_peek()->kind == TOK_AMP && lexer_peek_next()->kind == TOK_IDENT) {
						lexer_next(); /* & */
						STRNCPY(init_sym, lexer_peek()->text, sizeof(init_sym) - 1);
						lexer_next();
					} else if (value->kind == TOK_IDENT && (type->kind == TY_PTR || g->elem_size == 8)) {
					...
					if (init_sym[0] && g->elem_size < 8) {
						/*
						 * Function-pointer typedef arrays can arrive here with a scalar
						 * elem_size.  The symbol initializer proves this is pointer data;
						 * make the object pointer-sized so emit.c writes .quad entries.
						 */
						g->elem_size = 8;
					}

					for (int di = first_index; di <= last_index; di++) {
						g->init_values[di] = init_value;
						if (init_sym[0])
							STRNCPY(g->init_syms[di], init_sym, sizeof(g->init_syms[di]) - 1);
						else
							g->init_syms[di][0] = '\0';
					}

					if (last_index + 1 > g->init_count)
						g->init_count = last_index + 1;
					if (g->array_len == 0 && g->init_count > g->array_len)
						g->array_len = g->init_count;

					Debug(1, "GLOBAL_ARRAY_INIT name=%s designator=%d first=%d last=%d sym=%s value=%d count=%d\n",
					      g->name, has_designator, first_index, last_index,
					      init_sym[0] ? init_sym : "<none>", init_value, g->init_count);

...
void tcc_set_debug(int level)    { Debug_lvl = level; }
void tcc_set_warnings(int enabled) { warnings_enabled = enabled; }
void tcc_set_warnings_as_errors(int enabled) { warnings_as_errors = enabled; }
int tcc_warnings_enabled(void) { return warnings_enabled; }
int tcc_warnings_as_errors_enabled(void) { return warnings_as_errors; }

unsigned
tcc_hash_string(const char *s)
{
...
void
codegen_noop_call_args(int count, int fixed_params)
{
	(void)count;
	(void)fixed_params;
}
...
tcc_round_shift_u64(unsigned long long value, int shift)
{
	...
	base = value >> shift;
	rem = value & ((1ULL << shift) - 1ULL);
	half = 1ULL << (shift - 1);
	if (rem > half || (rem == half && (base & 1ULL)))
		base++;
	return base;
}

unsigned
tcc_double_bits_to_float_bits(unsigned long long bits)
{
	...
	if (fexp > 0) {
		rounded = tcc_round_shift_u64(mant, 29);
		if (rounded == (1ULL << 24)) {
			rounded >>= 1;
			fexp++;
			if (fexp >= 255)
		...
		return (sign << 31) | ((unsigned)fexp << 23) |
		       ((unsigned)rounded & 0x7fffffU);
	}

	if (e < -150)
		return sign << 31;
	rounded = tcc_round_shift_u64(mant, -(e + 97));
	if (rounded >= (1ULL << 23))
		return (sign << 31) | (1U << 23);
	return (sign << 31) | ((unsigned)rounded & 0x7fffffU);
}

void __attribute__((noreturn))
tcc_exit_failure(void)
{
	exit(1);
}
...
xrealloc_impl(void *ptr, size_t size, const char *file, int line, const char *func)
{
	...
	if (size > 0x10000000UL) {
		fprintf(stderr, "xrealloc_impl: suspicious size=%lu ptr=%p caller=%s:%d\n",
...
int 
tcc_strncmp(const char *a, const char *b, size_t n, const char *file, int line, const char *expr)
{
	unsigned long pa = (unsigned long)a;
	unsigned long pb = (unsigned long)b;
	...
	if (pa < 4096 || pb < 4096) {
		fprintf(stderr,
		        "%s:%d - STRNCMP bad args: a=%p b=%p expr=[%s]\n",
		        file,
		        line,
		        (void *)a,
		        (void *)b,
...
void
emit_raw_string_literal_len(const char *value, size_t len)
{
	size_t i = 0;
...
{
	emit_raw_string_literal_len(value, value ? strlen(value) : 0);
}

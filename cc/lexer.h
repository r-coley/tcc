...
typedef struct {
	...
	int	string_width; /* bytes per element for string literals: 1, 2, or 4 */
	...
	/* 64-bit numeric literal value (set alongside value for TOK_NUM).
	 * Use long_value when the literal needs more than 32 bits. */
	long          long_value;
	int           num_is_fp;
	int           num_is_float;
	int           num_is_unsigned;
